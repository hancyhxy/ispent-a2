...
// Type-picker → form → create. Emits a payload matching the backend Goal
// model (routes/goals.js validateGoal); the parent calls the API.
...
const TYPE_OPTIONS = [
  ...
  },
  {
    ...
    description: 'Cap spending in an expense category',
    icon: '🛡️',
    defaultIcon: '🍱',
  },
  {
...
export default function GoalFormModal({ isOpen, onClose, onCreate }) {
  ...
  const handleCreate = () => {
    ...
    if (type === 'savings') {
      ...
      onCreate({
        ...base,
        targetAmount: Number(targetAmount) || 0,
        // spending_limit aggregates expenses for this category; reuse the
        // category field as the tracked expense category.
        limitCategory: category.trim(),
        period: 'monthly',
      });
    } else {
      onCreate(base);
        ...
        <div className="p-6">
          ...
          <label className="block mb-4">
            <span className="text-xs font-medium text-text-secondary mb-1.5 block">
              {type === 'spending_limit' ? 'Expense category to track' : 'Category'}
            </span>
            <input
              type="text"
              value={category}
