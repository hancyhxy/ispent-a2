...
export default function MockGoalCard({ goal, onAddAmount, onToggleTodo, onDelete }) {
  if (goal.type === 'simple_todo') {
    return <TodoCard goal={goal} onToggleTodo={onToggleTodo} onDelete={onDelete} />;
  }
  return <ProgressCard goal={goal} onAddAmount={onAddAmount} onDelete={onDelete} />;
}

/* ═══════════════════════════════════════════════════════════
   PROGRESS CARD — savings + spending_limit
═══════════════════════════════════════════════════════════ */

function ProgressCard({ goal, onAddAmount, onDelete }) {
  ...
  const isNearComplete = goal.status === 'near_complete' || (pct >= 80 && pct < 100 && !overspent);
    ...
    <article className={cardClass}>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-xs text-text-muted">
          {goal.deadlineLabel || goal.periodLabel}
        </span>
      </div>

      {/* Footer: add input or button */}
      {!isCompleted && (
        <div className="mt-5">
          {showInput ? (
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={goal.type === 'savings' ? 'Amount to save' : 'Expense amount'}
                  autoFocus
            ...
            >
              <Plus size={14} />
              {goal.type === 'savings' ? 'Add to this goal' : 'Log expense'}
            </button>
          )}
        </div>
      )}

      {/* Sparkles for completed savings */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 ml-7">
            ...
            {isCompleted && goal.completedAt && (
              <span className="text-xs text-text-muted">· Completed {formatShortDate(goal.completedAt)}</span>
            )}
          </div>
        </div>
